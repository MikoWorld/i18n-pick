    ...
    return {
        visitor: {
            ...
            JSXExpressionContainer(path) {
                const { node } = path;
                detectWording(node.expression.value, path, 'jsx', 'JSXExpressionContainer');
              },
...
function detectWording(text, path, type, babelType) {
    if (typeof text == 'string' // sometimes text are null or non-string, this is a sanity check
        ...
        // rules out PascalCase
        && !/^[A-Z][a-z]*([A-Z][a-z]*)/.test(text)
        // rules out indentifiers like zswA_fefXvr, note that this assumes no space and at least one underscore in the whole text so it's unlikely to rule out natural languages by mistake
        && !/^(?=.*[-+_])\S+$/.test(text)
        // rules out colors
        && !/^#[0-9A-Fa-f]{6}$/.test(text)
        // rules out SVG path, by filtering out texts with number.number
        && !/\d\.\d/.test(text)
