...
function run(path) {
    glob(`${path}/**/*.{js,jsx,ts,tsx}`, { ignore: exclude.map(pattern => `${path}/${pattern}`) }, (error, files) => {
        ...
        fs.appendFile(`${targetDir}/en.json`, `${JSON.stringify([...zhCH.values()], null, '\t')}`, function (err) {
            if (err) {
                return console.error(err);
            }
...
function detectWording(text, path, type, babelType) {
    if (typeof text == 'string' // sometimes text are null or non-string, this is a sanity check
        ...
        /* 
        The SVG path detection is commented out as it does not work well. For example, it failed to extract the words "API Keys".
        For now, I simply added the word "Keys" as negative lookahead. However, many sentences are still not being extracted.
        I attempted to remove this SVG path detection, but the number of lines in the translation files increased significantly,
        to the point where it is not feasible to translate without ChatGPT API. Moreover, I do not know any easy way to detect SVG paths.
        */
        // rules out SVG path: 
        ...
        && !/^((?!\bthe\b|\band\b|\bin\b|\bKeys\b).)*([MmLlHhVvCcSsQqTtAaZz0-9.,\s]+)$/.test(text)
...
function report(text, path, type, babelType) {
    const { node } = path;
    const absolutePath = path.hub.file.opts.filename;
    const relativePath = path_module.relative(process.cwd(), absolutePath);
    const location = `${relativePath}#${node.loc ? node.loc.start.line : '!!!'}#${node.loc ? node.loc.start.column : '!!!'}`
                     + `#${node.loc ? node.loc.end.line : '!!!'}#${node.loc ? node.loc.end.column : '!!!'}`;
